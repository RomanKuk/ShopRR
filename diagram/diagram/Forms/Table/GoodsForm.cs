using diagram.Forms.InsernEdit;
using diagram.Forms.Users;
using System;
using System.Linq;
using System.Windows.Forms;

namespace diagram.Forms.Table
{
    public partial class GoodsForm : Form
    {
        my_db_for_db_2Entities db;
        public GoodsForm(my_db_for_db_2Entities _db)
        {
            InitializeComponent();
            db = _db;
        }

        private void GoodsForm_Load(object sender, EventArgs e)
        {
            makersBindingSource.DataSource = db.Makers.ToList();
            categoriesBindingSource.DataSource = db.Categories.ToList();
            goodsBindingSource.DataSource = db.Goods.ToList();

            Left = Top = 0;
            Width = Screen.PrimaryScreen.WorkingArea.Width;
            Height = Screen.PrimaryScreen.WorkingArea.Height;
            int barWidth = Width - panel4.Width;
            panel3.Width = panel5.Width = barWidth / 2;
        }

        private void Insert_Click(object sender, EventArgs e)
        {
            using (GoodsIEF frm = new GoodsIEF(null, db))
            {
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    goodsBindingSource.DataSource = db.Goods.ToList();
                }
            }
        }

        private void Edit_Click(object sender, EventArgs e)
        {
            if (goodsBindingSource.Current == null)
            {
                return;
            }
            using (GoodsIEF frm = new GoodsIEF(goodsBindingSource.Current as Goods, db))
            {
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    goodsBindingSource.DataSource = db.Goods.ToList();
                }
            }
        }

        private void Delete_Click(object sender, EventArgs e)
        {
            if (goodsBindingSource.Current != null)
            {
                if (MessageBox.Show("Are you sure", "Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    db.Goods.Remove(goodsBindingSource.Current as Goods);
                    goodsBindingSource.RemoveCurrent();
                    db.SaveChanges();
                }
            }
        }

        private void minimizeBtn_Click(object sender, EventArgs e)
        {
            this.WindowState = FormWindowState.Minimized;
        }

        private void closeBtn_Click(object sender, EventArgs e)
        {
            this.Close();
            Application.Exit();
        }

        private void backToHomeBtn_Click(object sender, EventArgs e)
        {
            this.Hide();
            Classes.FormState.PreviousPage = this;
            new TableControlForm(db).Show();
        }
    }
}
